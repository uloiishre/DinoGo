...
export function getOrderStars(orderId) {
  return response(stars.filter((star) => String(star.orderId) === String(orderId)))
}

export function updateStar(starId, request) {
  let star = stars.find((item) => String(item.starId) === String(starId))
  if (!star) {
    star = { starId: Number(starId), orderId: null, orderItemId: null, productId: 1, memberId: 1, reviewPriority: 1, starUpdAt: new Date().toISOString() }
    stars.push(star)
  }
  Object.assign(star, request, { starUpdAt: new Date().toISOString() })
  return response(star)
}

export function clearStar(starId) {
  const star = stars.find((item) => String(item.starId) === String(starId))
  if (star) Object.assign(star, { fiveStar: 0, feedback: '', imgOne: null, imgTwo: null, imgThree: null })
  return response(star)
}
