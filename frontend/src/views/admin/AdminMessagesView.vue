...
async function submitMessage() {
  ...
  const sendPayload = {
    ...
    msgtoMemberId: form.msgType === 'OC' ? recipientId : null,
...
<template>
  <section class="admin-message-page">
    ...
    <section v-else-if="activeTab === 'TEMPLATES'" ref="templatePanel" class="content-panel paged-panel">
      <header class="panel-toolbar"><div><h2>範本管理</h2><p>只顯示 send_status = SAVE。</p></div><div class="admin-template-toolbar"><label><span class="visually-hidden">篩選範本</span><select v-model="templateFilter" aria-label="篩選範本" :disabled="templateLoading" @change="changeFilter('template')"><option v-for="filter in templateFilters" :key="filter" :value="filter">{{ filter === 'ALL' ? '全部範本' : filter }}</option></select></label><button class="template-add-action" :disabled="templateLoading" @click="openTemplateCreate"><i class="bi bi-plus-lg" aria-hidden="true"></i>新增範本</button><button class="template-batch-delete" :disabled="templateLoading || selectedTemplateIds.size === 0" @click="deleteSelectedTemplates">批次刪除（{{ selectedTemplateIds.size }}）</button></div></header>
    ...
    <section v-else ref="messagePanel" class="content-panel paged-panel">
      ...
      <div v-else class="overview-list">
        <div class="overview-filters">
          ...
          <label><span class="visually-hidden">篩選訊息狀態</span><select v-model="overviewFilters.sendStatus" @change="activateOverviewFilter('status')"><option value="">—</option><option value="SEND">SEND</option><option value="SAVE">SAVE</option><option value="DELETE">DELETE</option></select><i class="bi bi-chevron-down" aria-hidden="true"></i></label>
          <span aria-hidden="true"></span>
        ...
        <div class="overview-columns"><span>send_id</span><span>msg_function</span><span>狀態</span><span>自訂範本名稱</span><span>from</span><span>ID</span><span>訊息標題 / 訊息內容</span><span>寄件時間</span></div>
