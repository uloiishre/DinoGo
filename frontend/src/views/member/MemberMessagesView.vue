...
const titles = {
  SYSTEM_INBOX: ['會員權益更新', '系統維護通知', '本週精選活動'],
  ORDER_INBOX: ['訂單已完成', '商品已出貨', '付款成功通知'],
  SELLER_INBOX: ['賣家回覆通知', '商品補貨通知', '商店優惠通知'],
}
const inboxes = reactive(Object.fromEntries(tabs.map((tab, tabIndex) => [tab.key,
  Array.from({ length: 36 }, (_, index) => ({
    recordId: tabIndex * 100 + index + 1,
    recordStatus: index % 4 === 0 ? 'READ' : 'UNREAD',
    sendTitle: titles[tab.key][index % 3],
    sendContent: `這是第 ${index + 1} 則${tab.label}，點擊後可查看訊息並更新為已讀狀態。`,
    recordCreatedAt: new Date(Date.now() - (tabIndex * 36 + index) * 3_600_000).toISOString(),
  })).sort((left, right) => new Date(right.recordCreatedAt) - new Date(left.recordCreatedAt)),
])))
...
function selectTab(key) { activeTab.value = key; currentPage.value = 1; selectedIds.value = new Set() }
...
function deleteSelected() {
  // 檢視版直接移除；正式版由後端設定決定軟刪除或硬刪除。
  inboxes[activeTab.value] = inboxes[activeTab.value].filter((message) => !selectedIds.value.has(message.recordId))
  selectedIds.value = new Set()
  currentPage.value = Math.min(currentPage.value, totalPages.value)
}
function openMessage(message) { message.recordStatus = 'READ'; selectedMessage.value = message }
function closeMessage() { selectedMessage.value = null }
  ...
  <main class="member-inbox-page container">
    ...
    <div v-if="selectedMessage" class="message-overlay" @click.self="closeMessage">
      <article class="message-dialog" role="dialog" aria-modal="true" aria-labelledby="preview-message-detail-title">
        ...
        <header><span class="message-dot" :class="{ read: selectedMessage.recordStatus === 'READ' }"></span><div><h2 id="preview-message-detail-title">{{ selectedMessage.sendTitle }}</h2><time :datetime="selectedMessage.recordCreatedAt">{{ formatDate(selectedMessage.recordCreatedAt) }}</time></div></header>
...
.message-overlay { position: fixed; z-index: 1050; inset: 0; display: grid; padding: var(--space-5); place-items: center; background: color-mix(in srgb, var(--color-text) 65%, transparent); }.message-dialog { position: relative; width: min(100%, 720px); padding: var(--space-6); background: var(--color-surface); border-radius: var(--radius-lg); box-shadow: var(--shadow-card); }.message-dialog__close { position: absolute; top: var(--space-3); right: var(--space-3); width: var(--space-7); height: var(--space-7); color: var(--color-text-muted); font-size: var(--font-size-xl); background: transparent; border: 0; }.message-dialog header { display: flex; gap: var(--space-3); padding-right: var(--space-7); }.message-dialog h2 { margin: 0 0 var(--space-1); }.message-dialog__content { margin-top: var(--space-5); padding-top: var(--space-5); white-space: pre-wrap; border-top: 1px solid var(--color-border); }
