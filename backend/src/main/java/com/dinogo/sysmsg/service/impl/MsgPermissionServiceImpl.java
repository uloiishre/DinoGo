...
public class MsgPermissionServiceImpl implements MsgPermissionService {
    ...
    public MsgPermissionServiceImpl(MemberClient members) { this.members = members; }

    @Override public Integer validateSeller(Integer loginMemberId) {
        MemberAuthResponse me = current(loginMemberId);
        if (me.getSellerId() == null) throw new SecurityException("目前登入會員不是商家");
        return me.getSellerId();
    }
    @Override public void validateSystemAdmin(Integer loginMemberId) {
        MemberAuthResponse me = current(loginMemberId);
        boolean adminRole = "ADMIN".equalsIgnoreCase(me.getRole()) || "SYSTEM_ADMIN".equalsIgnoreCase(me.getRole());
        if (!adminRole && !(Integer.valueOf(1).equals(me.getMemberId()) && Integer.valueOf(1).equals(me.getSellerId())))
    ...
    private MemberAuthResponse current(Integer assertedId) {
        MemberAuthResponse me = members.getProfile();
